...
{
  uint8_t frame_data[128];         // 要能放下最长的帧，用来存存放解析后的数据
  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        Speed_Ptr->set_pid_para_kp(p_temp);
        Speed_Ptr->set_pid_para_ki(i_temp);
        Speed_Ptr->set_pid_para_kd(d_temp);
      ...
      case SET_TARGET_CMD:
      {
        ...
        /* 在这里填写自己的处理函数 */
        ALGORITHM::Set_Target_Speed(actual_temp);
      }
      break;
...
//   HAL_UART_Transmit(&UartHandle, (uint8_t *)&sum, sizeof(sum), 0xFFFFF);                  // 发送校验和
    FIRE_SEND((uint8_t *)&set_packet,sizeof(set_packet));// 发送数据头
    FIRE_SEND((uint8_t *)data, num);// 发送参数
    FIRE_SEND((uint8_t *)&sum, sizeof(sum));// 发送校验和
}
/* 波形数据---------------------------------------------------------*/
...
void TOOLS_FIRE::Update()
{
    if(LOG_UART_AVAILABLE() > 0)
    {
        size_t data = FIRE_READ();//读取串口数据
        protocol_data_recv((uint8_t *)&data,1);//数据存在缓冲区
        receiving_process();//检查是否收到了完整帧的数据，有的话则处理
    } 
}

/*外部调用接口*/
/*工具初始化*/
void TOOLS_FIRE::Init()
{
    protocol_init();
}
void TOOLS_FIRE::Data_Recv(uint8_t *data, uint16_t data_len)
{
    protocol_data_recv(data, data_len);   
}
void TOOLS_FIRE::Data_Send(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
    set_computer_value(cmd,ch,data,num);
}
/**********************************************************************************************/
