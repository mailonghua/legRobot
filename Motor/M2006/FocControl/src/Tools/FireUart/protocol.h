...
// extern "C" {
// #endif   
extern PID * Speed_Ptr;//速度--用户接收到数据后进行设置
...
namespace TOOLS_FIRE
{
    void Init();
    void Update();
    /*发送*/
    void Data_Recv(uint8_t *data, uint16_t data_len);
    /*接收*/
    void Data_Send(uint8_t cmd, uint8_t ch, void *data, uint8_t num);
} 
